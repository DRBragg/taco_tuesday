...
RSpec.describe TacoTuesday do
  ...
  context 'emoji' do
    it "adds #🌮? to the Date class" do
      expect { Date.today.🌮? }.to_not raise_error
    end

    it "adds #🌮? to the Time class" do
      expect { Time.now.🌮? }.to_not raise_error
    end

    it "should retrun a Boolean value" do
      expect(Date.today.🌮?).to be(true).or(be(false))
      expect(Time.now.🌮?).to be(true).or(be(false))
    end

    it "should return true if Date is a tuesday" do
      tuesday = Date.new(2020, 5, 26) # May 26th 2020 is a Tuesday
      expect(tuesday.🌮?).to be true
    end

    it "should return false if Date is not a tuesday" do
      monday = Date.new(2020, 5, 25) # May 25th 2020 is a Monday
      expect(monday.🌮?).to be false
    end
